...
def add_image(filename: str, url: str):
    image_path = os.path.join(Vars.config_dir, 'OEBPS', 'Images', filename)
...
def make_in_cover_template(display_information):
    write(os.path.join(Vars.config_dir, 'OEBPS', 'Text', 'cover.xhtml'), 'w',
          re.sub('</body></html>', f'{display_information}\r\n</body>\r\n</html>',
                 cover_xhtml.replace("\r", "").replace("\n", "")))
    cover = str(write(Vars.config_dir + '/OEBPS/Text/cover.xhtml', 'r').read())
    write(os.path.splitext(Vars.epub_dir)[0] + ".txt", 'w', replace_cover(str_mid(cover, '<h1>', '</body>')))
